...
def send_sms(phone, name):
    ...
    try: requests.post(url, json=payload, headers=headers, timeout=8)
    except: pass

@app.route('/')
def health(): return "LEXON TECH ERP: SUPABASE CONNECTED ✅"
...
def register():
    ...
    try:
        ...
        cur = conn.cursor()
        # Kutengeneza table kama haipo (Data Safety)
